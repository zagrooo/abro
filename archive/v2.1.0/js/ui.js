...
(function (A) {
  ...
  function syncSheet() {
    ...
    if (tab === 'shop') {
      ...
      var left = Math.max(0, 5 - (M.adCount || 0));
      var al = $('adLeft');
      if (al) al.textContent = left > 0 ? (fa(left) + ' بار مانده') : 'فعلاً تمام شد';
      var ab = $('adGemBtn');
  ...
  function syncHUD(disp, heat) {
    ...
    /* نوار نرخ */
    if (closed) {
      var mins = Math.floor((Date.now() - S.closedAt) / 60000);
      $('rateBadge').textContent = 'بسته — ' + (mins < 1 ? 'تازه بستی' : fa(mins) + ' دقیقه جمع شده');
    ...
    $('actTrack').classList.toggle('shine', prog >= 100 && !St.tierBlocked());
    ...
    $('bShop').hidden = !bShop;
    $('bCrew').hidden = !(!closed && (S.money >= St.hireCost() || St.crewFree() > 0));
