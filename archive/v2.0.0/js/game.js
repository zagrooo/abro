...
(function (A) {
  ...
  var MODALS = ['shiftVeil', 'evVeil', 'rcVeil', 'actVeil', 'enVeil', 'intro', 'buyVeil'];
  function anyModal() {
  ...
  function watchAdForGems() {
    if ((M().adCount || 0) >= 5) {
      U.toast('امشب دیگر تبلیغی نیست. فردا برگرد.', 'bad');
      AU.sfx.no();
      return;
    }
  ...
  /* ═════════ حساب شب ═════════ */
  function runShift(hours, wasClosed) {
  ...
  function loop(now) {
    ...
    if (!booted) { requestAnimationFrame(loop); return; }
    var open = !anyModal();
