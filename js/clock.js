...
(function (A) {
  ...
  /* ───────── منحنی تقاضا ─────────
     جدول می‌تواند عوض شود (مثلاً در رمضان وارونه می‌شود)،
     پس مرتب‌شده‌اش را کش می‌کنیم ولی به مرجع جدول گره می‌زنیم. */
  var cacheSrc = null, cacheSorted = null;
  function sorted() {
    var src = (A.world && A.world.demandTable) ? A.world.demandTable() : D.DEMAND;
    if (src !== cacheSrc) {
      cacheSrc = src;
      cacheSorted = src.slice().sort(function (a, b) { return a.from - b.from; });
    }
    return cacheSorted;
  }
